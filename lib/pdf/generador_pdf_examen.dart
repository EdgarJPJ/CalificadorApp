...
class GeneradorPdfExamen {
  ...
  Future<Uint8List> construirDocumento({
    ...
    final disenosDePaginas = _organizarPaginas(materias: materias, anchoPagina: anchoPagina, altoContenido: altoContenido, cantidadColumnas: cantidadColumnas);
      ...
      for (final diseno in disenosDePaginas) {
        pdf.addPage(
          pw.Page(
            ...
            build: (context) => _ensamblarPagina(nombreAlumno: nombreAlumno, esClaveMaestro: esClaveMaestro, diseno: diseno),
          ),
        );
      }
    }
  ...
  List<DisenoPagina> _organizarPaginas({required List<Materia> materias, required double anchoPagina, required double altoContenido, required int cantidadColumnas}) {
    final paginas = <DisenoPagina>[];
    var paginaActual = DisenoPagina(anchoPagina: anchoPagina, cantidadColumnas: cantidadColumnas);

    for (final materia in materias) {
      final bloque = BloqueMateria.desdeMateria(materia: materia, anchoPagina: anchoPagina, cantidadColumnas: cantidadColumnas);
  ...
  pw.Widget _ensamblarPagina({required String nombreAlumno, required bool esClaveMaestro, required DisenoPagina diseno}) {
    final offsetX = math.max(0.0, (diseno.anchoPagina - diseno.maximoDerecha) / 2.0);

    return pw.Stack(
      children: [
        pw.Positioned(left: 6, top: 6, child: ComponentesExamen.marcador()),
        pw.Positioned(right: 6, top: 6, child: ComponentesExamen.marcador()),
        pw.Positioned(left: 6, bottom: 40, child: ComponentesExamen.marcador()),
        pw.Positioned(right: 6, bottom: 40, child: ComponentesExamen.marcador()),
        pw.Column(
          children: [
            pw.Padding(
              ...
              child: pw.Column(
                ...
                children: ComponentesExamen.construirEncabezado(nombreAlumno: nombreAlumno, esClaveMaestro: esClaveMaestro),
              ),
            ),
              ...
              pw.SizedBox(
                ...
                child: pw.Stack(
                  children: diseno.bloques.map((b) => pw.Positioned(
                    ...
                    top: b.arriba,
                    child: pw.SizedBox(width: b.bloque.anchoBloque, child: ComponentesExamen.construirBloqueMateria(b.bloque)),
                  )).toList(),
                ),
              ),
          ],
